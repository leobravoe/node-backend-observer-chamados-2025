...
-- Esta função pega os dados da linha modificada e os envia como uma notificação JSON.
CREATE OR REPLACE FUNCTION notify_chamado_change()
  RETURNS TRIGGER AS $$
DECLARE
  -- Declara uma variável para o payload da notificação
  payload JSON;
BEGIN
  -- TG_OP contém a operação que disparou o gatilho (INSERT, UPDATE, DELETE)
  -- NEW é uma variável especial que contém a nova linha (em INSERT/UPDATE)
  -- OLD é uma variável especial que contém a linha antiga (em UPDATE/DELETE)
  payload = json_build_object(
    'operation', TG_OP,
    'record', COALESCE(NEW, OLD) -- Usa NEW se existir, senão usa OLD
  );

  -- 2. Envia a notificação no canal 'chamados_channel' com o payload.
  -- Todas as aplicações que estiverem escutando ('LISTEN') este canal receberão a mensagem.
  PERFORM pg_notify('chamados_channel', payload::text);

  RETURN NEW;
END;
$$ LANGUAGE plpgsql;

-- 3. Cria o gatilho na tabela "Chamados".
-- Ele será disparado DEPOIS de qualquer INSERT, UPDATE ou DELETE.
CREATE TRIGGER chamados_trigger
  AFTER INSERT OR UPDATE OR DELETE ON Chamados
  FOR EACH ROW EXECUTE FUNCTION notify_chamado_change();