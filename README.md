# Observer Chamados - Backend

## Visão Geral

Este projeto é o serviço de backend para a aplicação **Observer Chamados**, projetado para estudantes de desenvolvimento web. Ele fornece uma API REST para gerenciar "chamados" (tickets) e utiliza Server-Sent Events (SSE) para notificar os clientes sobre atualizações em tempo real.

Em alto nível, a arquitetura consiste em uma aplicação Node.js que utiliza o framework Express para expor os endpoints da API. O serviço se conecta a um banco de dados PostgreSQL para persistência de dados. A funcionalidade de tempo real é implementada através de uma conexão dedicada que escuta notificações diretamente do banco de dados (`LISTEN/NOTIFY`) e as retransmite para os clientes conectados via SSE.
...
2.  Navegue até o diretório do projeto:
    ```sh
    cd node-backend-observer-chamados-2025
    ```
...
4.  **Variáveis de Ambiente**: Crie um arquivo `.env` na raiz do projeto. As seguintes variáveis são necessárias para a conexão com o banco de dados e para a configuração do servidor. Não há valores padrão definidos no código.
...
**Fontes**: `package.json`, `src/server/server.js`, `src/database/database.js`, `src/services/sseService.js`
...
As mesmas variáveis de ambiente da seção de configuração são necessárias em produção.
...
**Fonte**: `src/database/database.js`, `src/server/server.js`
